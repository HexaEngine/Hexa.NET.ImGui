namespace Example.ImGuiDemo
{
    ...
    internal class ImGuiInputHandler : IDisposable
    {
        ...
        /*
        private static unsafe byte* GetClipboardText(void* user_data)
        {
        ...
        private static unsafe void SetClipboardText(void* user_data, byte* text)
        {
            Clipboard.SetClipboardText(text);
        }
        */

        private unsafe void KeyboardCharInput(object? sender, KeyboardCharEventArgs e)
        {
            var io = ImGui.GetIO();
            ImGui.AddInputCharacterUTF16(io, e.Char);
        }

        private unsafe void KeyboardInput(object? sender, KeyboardEventArgs e)
        {
            var io = ImGui.GetIO();
        ...
        private unsafe void MouseButtonInput(object? sender, MouseButtonEventArgs e)
        {
            var io = ImGui.GetIO();
