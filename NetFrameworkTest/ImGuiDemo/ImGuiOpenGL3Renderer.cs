...
namespace ExampleD3D11.ImGuiDemo
{
    ...
    public static unsafe class ImGuiOpenGL3Renderer
    {
        ...
        public static bool Init(GL gl, byte* glsl_version)
        {
            GL = gl;
    ...
    bd->GlProfileIsCompat = (bd->GlProfileMask & GL_CONTEXT_COMPATIBILITY_PROFILE_BIT) != 0;
        ...
        Debug.WriteLine($"GlVersion = {bd.GlVersion}, \"{glVersionStr}\"\nGlProfileIsCompat = {bd.GlProfileIsCompat}\nGlProfileMask = 0x{bd.GlProfileMask:X}\nGlProfileIsES2 = {bd.GlProfileIsES2}\nGlProfileIsES3 = {bd.GlProfileIsES3}\nGL_VENDOR = '{Marshal.PtrToStringAnsi((IntPtr)_gl.GetString(StringName.Vendor))}'\nGL_RENDERER = '{Marshal.PtrToStringAnsi((IntPtr)_gl.GetString(StringName.Renderer))}'");
            ...
            if (string.IsNullOrEmpty(gl_version_str))
            {
#if IMGUI_IMPL_OPENGL_ES2
            gl_version_str = "#version 100";
#elif IMGUI_IMPL_OPENGL_ES3
            gl_version_str = "#version 300 es";
#elif __APPLE__
            gl_version_str = "#version 150";
#else
    ...
    if (!clip_origin_lower_left) { float tmp = T; T = B; B = tmp; } // Swap top and bottom if origin is upper left
        ...
        private static void RenderDrawData(ImDrawData* draw_data)
        {
            ...
            for (int n = 0; n < draw_data->CmdListsCount; n++)
            {
                ...
                nuint idx_buffer_size = (nuint)cmd_list->IdxBuffer.Size * (nuint)sizeof(ImDrawIdx);
                if (bd->UseBufferSubData)
                {
                    if (bd->VertexBufferSize < vtx_buffer_size)
                    {
                        ...
                        /*GL_CALL*/
                        GL.BufferData(GLEnum.ArrayBuffer, (nuint)bd->VertexBufferSize, null, GLEnum.StreamDraw);
                    }
                    if (bd->IndexBufferSize < idx_buffer_size)
                    {
                        bd->IndexBufferSize = idx_buffer_size;
                        /*GL_CALL*/
                        GL.BufferData(GLEnum.ElementArrayBuffer, (nuint)bd->IndexBufferSize, null, GLEnum.StreamDraw);
                    }
                    /*GL_CALL*/
                    GL.BufferSubData(GLEnum.ArrayBuffer, 0, (nuint)vtx_buffer_size, (void*)cmd_list->VtxBuffer.Data);
                    /*GL_CALL*/
...
";
        ...
        private static void RenderWindow(nint pViewport, nint v)
        {
