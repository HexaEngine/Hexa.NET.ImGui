namespace ExampleD3D11.Mathmatics
{
    ...
    public static class MathUtil
    {
        ...
        public const float PIDIV2 = (float)(Math.PI / 2);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static int ComputeMipLevels(int width, int height)
        {
            return (int)Math.Log(Math.Max(width, height), 2);
        }
        ...
        {
            yaw = (float)Math.Atan2(2.0f * (r.Y * r.W + r.X * r.Z), 1.0f - 2.0f * (r.X * r.X + r.Y * r.Y));
            pitch = (float)Math.Asin(2.0f * (r.X * r.W - r.Y * r.Z));
            roll = (float)Math.Atan2(2.0f * (r.X * r.Y + r.Z * r.W), 1.0f - 2.0f * (r.X * r.X + r.Z * r.Z));
        }
        ...
        {
            return new((float)Math.Floor(vector.X), (float)Math.Floor(vector.Y), (float)Math.Floor(vector.Z));
        }
        ...
        {
            float x = (float)Math.Atan2(2.0f * (r.Y * r.W + r.X * r.Z), 1.0f - 2.0f * (r.X * r.X + r.Y * r.Y)) * mask.X;
            float y = (float)Math.Asin(2.0f * (r.X * r.W - r.Y * r.Z)) * mask.Y;
            float z = (float)Math.Atan2(2.0f * (r.X * r.Y + r.Z * r.W), 1.0f - 2.0f * (r.X * r.X + r.Z * r.Z)) * mask.Z;

            float xHalf = x * 0.5f;
            float yHalf = y * 0.5f;
            float zHalf = z * 0.5f;

            float cx = (float)Math.Cos(xHalf);
            float cy = (float)Math.Cos(yHalf);
            float cz = (float)Math.Cos(zHalf);
            float sx = (float)Math.Sin(xHalf);
            float sy = (float)Math.Sin(yHalf);
            float sz = (float)Math.Sin(zHalf);
